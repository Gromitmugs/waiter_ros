...
class Navigator():
    ...
    def __init__(self):
        ...
        rospy.Subscriber("nav_location", Pose, self.callback)
    ...
    def callback(self, navLocation):
        rospy.set_param('ROBOT_STATUS', '1') # assuming 1 is not free
        print("NAVIGATING!")
        if navLocation.name == 'KITCHEN' :
            position = {'x': navLocation.position.x,
                    'y': navLocation.position.y}
            quaternion = {'r1':navLocation.orientation.x,'r2':navLocation.orientation.y, 'r3':navLocation.orientation.z, 'r4':navLocation.orientation.w}
            rospy.loginfo("Going to %s at (%s, %s) pose", navLocation.name, position['x'], position['y'])
            success = self.goto(position, quaternion)

            if success:
                rospy.loginfo("Successfully Reached {0}".format(navLocation.name))
            else:
                rospy.loginfo("The base failed to reach the desired pose at destination")

        elif navLocation.name == 'TABLE' :
            position = {'x': navLocation.position.x,
                    'y': navLocation.position.y}
            quaternion = {'r1':navLocation.orientation.x,'r2':navLocation.orientation.y, 'r3':navLocation.orientation.z, 'r4':navLocation.orientation.w}
            rospy.loginfo("Going to %s at (%s, %s) pose", navLocation.name, position['x'], position['y'])
